        ...
        <div className="space-y-6">
            ...
            <div className="bg-slate-50 dark:bg-slate-700/30 rounded-lg p-6 border">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div><label className="block text-sm font-medium mb-1 text-slate-700 dark:text-slate-300">Establishment Name</label><input type="text" value={settings.pfEstablishmentName || ''} onChange={e => setSettings({...settings, pfEstablishmentName: e.target.value})} className="w-full px-3 py-2 border rounded-lg bg-white dark:bg-slate-800" /></div>
                </div>
            </div>
            <div className="bg-slate-50 dark:bg-slate-700/30 rounded-lg p-6 border">
                <h3 className="font-medium mb-4 text-slate-900 dark:text-white">Contribution Rates</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div><label className="block text-sm font-medium mb-1 text-slate-700 dark:text-slate-300">Employee %</label><input type="number" step="0.01" value={settings.pfEmployeeRate || 12} onChange={e => setSettings({...settings, pfEmployeeRate: parseFloat(e.target.value)})} className="w-full px-3 py-2 border rounded-lg bg-white dark:bg-slate-800" /></div>
                    <div><label className="block text-sm font-medium mb-1 text-slate-700 dark:text-slate-300">Employer %</label><input type="number" step="0.01" value={settings.pfEmployerRate || 12} onChange={e => setSettings({...settings, pfEmployerRate: parseFloat(e.target.value)})} className="w-full px-3 py-2 border rounded-lg bg-white dark:bg-slate-800" /></div>
                    <div><label className="block text-sm font-medium mb-1 text-slate-700 dark:text-slate-300">Admin %</label><input type="number" step="0.01" value={settings.pfAdminChargesRate || 0.5} onChange={e => setSettings({...settings, pfAdminChargesRate: parseFloat(e.target.value)})} className="w-full px-3 py-2 border rounded-lg bg-white dark:bg-slate-800" /></div>
                    <div><label className="block text-sm font-medium mb-1 text-slate-700 dark:text-slate-300">Wage Ceiling ₹</label><input type="number" value={settings.pfWageCeiling || 15000} onChange={e => setSettings({...settings, pfWageCeiling: parseInt(e.target.value)})} className="w-full px-3 py-2 border rounded-lg bg-white dark:bg-slate-800" /></div>
                </div>
            </div>
        </div>
    );
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-slate-50 dark:bg-slate-700/30 rounded-lg p-6 border">
                ...
                <div><label className="block text-sm font-medium mb-1 text-slate-700 dark:text-slate-300">ESI Code</label><input type="text" value={settings.esiCode || ''} onChange={e => setSettings({...settings, esiCode: e.target.value})} placeholder="12345678901234567" className="w-full md:w-1/2 px-3 py-2 border rounded-lg bg-white dark:bg-slate-800" /></div>
            </div>
            <div className="bg-slate-50 dark:bg-slate-700/30 rounded-lg p-6 border">
                <h3 className="font-medium mb-4 text-slate-900 dark:text-white">Rates</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div><label className="block text-sm font-medium mb-1 text-slate-700 dark:text-slate-300">Employee %</label><input type="number" step="0.01" value={settings.esiEmployeeRate || 0.75} onChange={e => setSettings({...settings, esiEmployeeRate: parseFloat(e.target.value)})} className="w-full px-3 py-2 border rounded-lg bg-white dark:bg-slate-800" /></div>
                    <div><label className="block text-sm font-medium mb-1 text-slate-700 dark:text-slate-300">Employer %</label><input type="number" step="0.01" value={settings.esiEmployerRate || 3.25} onChange={e => setSettings({...settings, esiEmployerRate: parseFloat(e.target.value)})} className="w-full px-3 py-2 border rounded-lg bg-white dark:bg-slate-800" /></div>
                    <div><label className="block text-sm font-medium mb-1 text-slate-700 dark:text-slate-300">Wage Ceiling ₹</label><input type="number" value={settings.esiWageCeiling || 21000} onChange={e => setSettings({...settings, esiWageCeiling: parseInt(e.target.value)})} className="w-full px-3 py-2 border rounded-lg bg-white dark:bg-slate-800" /></div>
                </div>
            </div>
        </div>
    );
        ...
        <div className="space-y-6">
            ...
            <div className="bg-slate-50 dark:bg-slate-700/30 rounded-lg p-6 border">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div><label className="block text-sm font-medium mb-1 text-slate-700 dark:text-slate-300">View Slabs For</label><select value={selectedPtState} onChange={e => setSelectedPtState(e.target.value)} className="w-full px-3 py-2 border rounded-lg bg-white dark:bg-slate-800">{(ptStates.length > 0 ? ptStates : ['Tamil Nadu', 'Karnataka', 'Maharashtra']).map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                </div>
            </div>
            <div className="bg-slate-50 dark:bg-slate-700/30 rounded-lg p-6 border">
