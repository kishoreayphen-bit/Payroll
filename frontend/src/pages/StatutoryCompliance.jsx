    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-slate-50 dark:bg-slate-700/30 rounded-lg p-6 border">
                <h3 className="font-medium mb-4">Establishment Details</h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div><label className="block text-sm font-medium mb-1">PF Establishment ID</label><input type="text" value={settings.pfEstablishmentId || ''} onChange={e => setSettings({...settings, pfEstablishmentId: e.target.value.toUpperCase()})} placeholder="TNCHE1234567000" className="w-full px-3 py-2 border rounded-lg bg-white dark:bg-slate-800" /></div>
                    <div><label className="block text-sm font-medium mb-1">Establishment Name</label><input type="text" value={settings.pfEstablishmentName || ''} onChange={e => setSettings({...settings, pfEstablishmentName: e.target.value})} className="w-full px-3 py-2 border rounded-lg bg-white dark:bg-slate-800" /></div>
                </div>
            </div>
            <div className="bg-slate-50 dark:bg-slate-700/30 rounded-lg p-6 border">
                <h3 className="font-medium mb-4">Contribution Rates</h3>
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    <div><label className="block text-sm font-medium mb-1">Employee %</label><input type="number" step="0.01" value={settings.pfEmployeeRate || 12} onChange={e => setSettings({...settings, pfEmployeeRate: parseFloat(e.target.value)})} className="w-full px-3 py-2 border rounded-lg bg-white dark:bg-slate-800" /></div>
                    <div><label className="block text-sm font-medium mb-1">Employer %</label><input type="number" step="0.01" value={settings.pfEmployerRate || 12} onChange={e => setSettings({...settings, pfEmployerRate: parseFloat(e.target.value)})} className="w-full px-3 py-2 border rounded-lg bg-white dark:bg-slate-800" /></div>
                    <div><label className="block text-sm font-medium mb-1">Admin %</label><input type="number" step="0.01" value={settings.pfAdminChargesRate || 0.5} onChange={e => setSettings({...settings, pfAdminChargesRate: parseFloat(e.target.value)})} className="w-full px-3 py-2 border rounded-lg bg-white dark:bg-slate-800" /></div>
                    <div><label className="block text-sm font-medium mb-1">Wage Ceiling ₹</label><input type="number" value={settings.pfWageCeiling || 15000} onChange={e => setSettings({...settings, pfWageCeiling: parseInt(e.target.value)})} className="w-full px-3 py-2 border rounded-lg bg-white dark:bg-slate-800" /></div>
                </div>
            </div>
        </div>
    );
...
function ESITab({ settings, setSettings, saveSettings, saving }) {
    return (
        <div className="space-y-6">
            ...
            <div className="bg-slate-50 dark:bg-slate-700/30 rounded-lg p-6 border">
                <h3 className="font-medium mb-4">ESI Registration</h3>
                <div><label className="block text-sm font-medium mb-1">ESI Code</label><input type="text" value={settings.esiCode || ''} onChange={e => setSettings({...settings, esiCode: e.target.value})} placeholder="12345678901234567" className="w-full md:w-1/2 px-3 py-2 border rounded-lg bg-white dark:bg-slate-800" /></div>
            </div>
            <div className="bg-slate-50 dark:bg-slate-700/30 rounded-lg p-6 border">
                <h3 className="font-medium mb-4">Rates</h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div><label className="block text-sm font-medium mb-1">Employee %</label><input type="number" step="0.01" value={settings.esiEmployeeRate || 0.75} onChange={e => setSettings({...settings, esiEmployeeRate: parseFloat(e.target.value)})} className="w-full px-3 py-2 border rounded-lg bg-white dark:bg-slate-800" /></div>
                    <div><label className="block text-sm font-medium mb-1">Employer %</label><input type="number" step="0.01" value={settings.esiEmployerRate || 3.25} onChange={e => setSettings({...settings, esiEmployerRate: parseFloat(e.target.value)})} className="w-full px-3 py-2 border rounded-lg bg-white dark:bg-slate-800" /></div>
                    <div><label className="block text-sm font-medium mb-1">Wage Ceiling ₹</label><input type="number" value={settings.esiWageCeiling || 21000} onChange={e => setSettings({...settings, esiWageCeiling: parseInt(e.target.value)})} className="w-full px-3 py-2 border rounded-lg bg-white dark:bg-slate-800" /></div>
                </div>
            </div>
        </div>
    );
...
function PTTab({ settings, setSettings, ptSlabs, ptStates, selectedPtState, setSelectedPtState, initializePTSlabs, formatCurrency }) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="bg-slate-50 dark:bg-slate-700/30 rounded-lg p-6 border">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div><label className="block text-sm font-medium mb-1">Organization State</label><select value={settings.ptState || 'Tamil Nadu'} onChange={e => { setSettings({...settings, ptState: e.target.value}); setSelectedPtState(e.target.value); }} className="w-full px-3 py-2 border rounded-lg bg-white dark:bg-slate-800">{(ptStates.length > 0 ? ptStates : ['Tamil Nadu', 'Karnataka', 'Maharashtra', 'Andhra Pradesh', 'Telangana', 'West Bengal', 'Gujarat', 'Kerala']).map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                    <div><label className="block text-sm font-medium mb-1">View Slabs For</label><select value={selectedPtState} onChange={e => setSelectedPtState(e.target.value)} className="w-full px-3 py-2 border rounded-lg bg-white dark:bg-slate-800">{(ptStates.length > 0 ? ptStates : ['Tamil Nadu', 'Karnataka', 'Maharashtra']).map(s => <option key={s} value={s}>{s}</option>)}</select></div>
                </div>
            </div>
            <div className="bg-slate-50 dark:bg-slate-700/30 rounded-lg p-6 border">
                <h3 className="font-medium mb-4">PT Slabs - {selectedPtState}</h3>
                {filteredSlabs.length > 0 ? (
                    <table className="w-full"><thead className="bg-slate-100 dark:bg-slate-700"><tr><th className="px-4 py-3 text-left text-xs font-semibold">From (₹)</th><th className="px-4 py-3 text-left text-xs font-semibold">To (₹)</th><th className="px-4 py-3 text-right text-xs font-semibold">Tax (₹)</th></tr></thead>
                    <tbody className="divide-y">{filteredSlabs.map((s, i) => <tr key={i} className="hover:bg-slate-100/50"><td className="px-4 py-3 text-sm">{formatCurrency(s.fromAmount)}</td><td className="px-4 py-3 text-sm">{s.toAmount ? formatCurrency(s.toAmount) : 'Above'}</td><td className="px-4 py-3 text-sm text-right font-medium">{formatCurrency(s.taxAmount)}</td></tr>)}</tbody></table>
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                ...
                <select value={financialYear} onChange={e => setFinancialYear(e.target.value)} className="px-3 py-2 border rounded-lg bg-white dark:bg-slate-800 text-sm"><option value="2025-26">FY 2025-26</option><option value="2024-25">FY 2024-25</option></select>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="bg-slate-50 dark:bg-slate-700/30 rounded-lg p-6 border">
                <h3 className="font-medium mb-4">Tax Declarations</h3>
                {taxDeclarations.length > 0 ? (
                    ...
                    <tbody className="divide-y">{taxDeclarations.map(d => <tr key={d.id} className="hover:bg-slate-100/50"><td className="px-4 py-3 text-sm">{d.employeeName}</td><td className="px-4 py-3"><span className={`px-2 py-1 rounded-full text-xs font-medium ${d.taxRegime === 'NEW' ? 'bg-green-100 text-green-700' : 'bg-blue-100 text-blue-700'}`}>{d.taxRegime || 'NEW'}</span></td><td className="px-4 py-3 text-sm text-right">{formatCurrency(d.total80C)}</td><td className="px-4 py-3 text-center"><span className={`px-2 py-1 rounded-full text-xs ${d.status === 'SUBMITTED' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'}`}>{d.status || 'DRAFT'}</span></td></tr>)}</tbody></table>
...
function EmployeesTab({ employees, searchTerm, setSearchTerm, loadEmployees }) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="relative"><Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" /><input type="text" placeholder="Search employees..." value={searchTerm} onChange={e => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-2 border rounded-lg bg-white dark:bg-slate-800" /></div>
            <div className="bg-slate-50 dark:bg-slate-700/30 rounded-lg border overflow-hidden">
                {filtered.length > 0 ? (
                    <table className="w-full"><thead className="bg-slate-100"><tr><th className="px-4 py-3 text-left text-xs font-semibold">Employee</th><th className="px-4 py-3 text-left text-xs font-semibold">UAN</th><th className="px-4 py-3 text-left text-xs font-semibold">ESI</th><th className="px-4 py-3 text-center text-xs font-semibold">PF</th><th className="px-4 py-3 text-center text-xs font-semibold">ESI</th><th className="px-4 py-3 text-center text-xs font-semibold">PT</th><th className="px-4 py-3 text-center text-xs font-semibold">Regime</th></tr></thead>
                    <tbody className="divide-y">{filtered.map(e => <tr key={e.id} className="hover:bg-slate-100/50"><td className="px-4 py-3"><p className="text-sm font-medium">{e.employeeName}</p><p className="text-xs text-slate-500">{e.employeeCode}</p></td><td className="px-4 py-3 text-sm">{e.uanNumber || '-'}</td><td className="px-4 py-3 text-sm">{e.esiNumber || '-'}</td><td className="px-4 py-3 text-center">{e.isPfApplicable ? <CheckCircle className="w-4 h-4 text-green-500 mx-auto" /> : <AlertCircle className="w-4 h-4 text-slate-300 mx-auto" />}</td><td className="px-4 py-3 text-center">{e.isEsiApplicable ? <CheckCircle className="w-4 h-4 text-green-500 mx-auto" /> : <AlertCircle className="w-4 h-4 text-slate-300 mx-auto" />}</td><td className="px-4 py-3 text-center">{e.isPtApplicable ? <CheckCircle className="w-4 h-4 text-green-500 mx-auto" /> : <AlertCircle className="w-4 h-4 text-slate-300 mx-auto" />}</td><td className="px-4 py-3 text-center"><span className={`px-2 py-1 rounded text-xs ${e.taxRegime === 'OLD' ? 'bg-blue-100 text-blue-700' : 'bg-green-100 text-green-700'}`}>{e.taxRegime || 'NEW'}</span></td></tr>)}</tbody></table>
